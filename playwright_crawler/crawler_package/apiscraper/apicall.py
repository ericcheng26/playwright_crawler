...
class APICall:
    ...
    def __json__(self):
        jsonDict = {"original": self.originalUrl,
                    "base": self.base, "path": self.path}
        ...
        jsonDict["responseSizes"] = 0 if len(
            self.returnSizes) == 0 else int(mean(self.returnSizes))
    ...
    def toHTML(self):
        ...
        if len(self.params) > 0:
            ...
            htmlVal += "</table><p>"
        htmlVal += "<br><b>Example:</b> <a href=\""+self.originalUrl + \
            "\ target=\"_blank\">"+self.originalUrl+"</a></br>"
    ...
    def removeUnneededParameters(self):
        ...
        baseline = requests.get(
            str(self.base)+str(self.path), params=params).text
        for key in params.keys():
            newParams = dict(params)
            del newParams[key]
            testText = requests.get(
                str(self.base)+str(self.path), params=newParams).text
            ...
            if self.path == call.path and self.base == call.base:
                ...
                for key, vals in self.params.items():
                    if key not in call.params:
                        ...
                        call.params[key] = list(
                            set(call.params[key] + self.params[key]))
                return apiCalls
