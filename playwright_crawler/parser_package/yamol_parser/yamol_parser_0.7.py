...
#############################以下為內部方法的實作##########################################
    ...
    def _get_choice_question(self, bstag):
        ...
        qid_re = re.search(r'(\d+)\s*[.]', bstag.getText())
    ...
    '''
        ...
        div_open = r'<div style="display: table; table-layout: fixed; width: -webkit-fill-available; border: 1px solid; border-radius: 40px; border-color: #b58097; padding: 8px 8px 11px 20px; margin-top: 7px; background-color:AliceBlue; font-size: 12pt; page-break-inside:avoid; -webkit-box-decoration-break: clone;" class=<class>>'
...
def quick_yamol_parser(html_dir, parsed_dir):
    # 取出所有 html 檔，轉成 .json
    html_dir_list = listdir(html_dir)
    parsed_path_list = [
        join(
            join(parsed_dir,
        ...
        for p in html_dir_list if '.html' in basename(p)
    ]
